...
function PatientParticular(props) {
  ...
  const getOtp = () => {
    GetPatientOTP(data.demographic_map.pid, setOtp, setOtpLoading, viewOtp);
  };
    ...
    <div className="patientParticularMain">
      ...
      <Row>
        ...
        <Col span={24} style={{ marginTop: "1rem" }}>
          <ButtonRow
            deviceType={
              data.demographic_map?.patch_patient_maps[0]?.patches[0]
                ?.patch_type
            }
            viewOtp={viewOtp}
            getOtp={getOtp}
      ...
      {/*  NOTES SECTION */}
      {data.demographic_map.note && <NotesSection data={data} />}
      ...
      >
        <Tabs defaultActiveKey="1">
          ...
          <TabPane tab="EWS Table" key="2">
            <EwsTable pid={data.demographic_map.pid} />
          </TabPane>
        </Tabs>
      </div>
    </div>
