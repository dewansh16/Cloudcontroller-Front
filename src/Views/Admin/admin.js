...
export default function Admin() {
    ...
    useEffect(() => {
        ...
        tenantApi
            .getTenantList()
            .then((res) => {
                const myTenant = res.data?.response?.tenants?.find(tenant => tenant?.tenant_uuid === user?.tenant)
                setTenants(res.data?.response.tenants);

                setcurrentTenant({
                    uuid: myTenant.tenant_uuid,
                    name: myTenant.tenant_name,
                });
                getLocation(myTenant.tenant_uuid);
                setChangeui(true);
                setIsInfoPage(true);
