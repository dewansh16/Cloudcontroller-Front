...
const RespRatechart = (props) => {
  let rrData;
  let minval, maxval;
  let minel, maxel;
  if (props.hourlyChartData === undefined) {
    minval = props.chartData.rr[0].value;
    maxval = props.chartData.rr[0].value;
    minel = props.chartData.rr[0];
    maxel = props.chartData.rr[0];
    props.chartData.rr.forEach((item) => {
    ...
    rrData = props.chartData.rr.map((item) => {
      const monthNames = [
        "January",
        "February",
        "March",
    ...
    rrData = props.hourlyChartData.rr.map((item) => {
      minval = props.hourlyChartData.rr[0].value;
      maxval = props.hourlyChartData.rr[0].value;
      minel = props.hourlyChartData.rr[0];
      maxel = props.hourlyChartData.rr[0];
      props.hourlyChartData.rr.forEach((item) => {
        if (minval > item.value) {
          minel = item;
          minval = item.value;
        }
  ...
  const config = {
    ...
    tooltip: {
      customContent: (title, items) => {
        return (
          <>
            <h5 style={{ marginTop: 16 }}>Resp Rate</h5>
